...
def anon_token():
    r = requests.post(f"{BASE_URL}/api/auth/anonymous", json={"nickname": f"TEST_iter158_{os.getpid()}"}, timeout=15)
...
class TestStoriesEndpoints:
    ...
    def test_stories_view_invalid_id_404_or_400(self, auth_headers):
        ...
        r = requests.post(f"{BASE_URL}/api/stories/nonexistent-id-iter158/view", headers=auth_headers, timeout=15)
        assert r.status_code in (400, 404, 422), f"expected 4xx, got {r.status_code}: {r.text[:200]}"


class TestAuthSmoke:
    def test_anonymous_signup(self):
        r = requests.post(f"{BASE_URL}/api/auth/anonymous", json={"nickname": f"TEST_iter158smoke_{os.getpid()}"}, timeout=15)
        assert r.status_code == 200, f"{r.status_code} {r.text[:200]}"
        data = r.json()
